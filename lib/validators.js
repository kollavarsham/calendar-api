...
const isNumeric = (possibleNumber) => {
  return !isNaN(parseFloat(possibleNumber)) && isFinite(possibleNumber);
};

const isInteger = (possibleInteger) => {
  return !isNaN(possibleInteger) && ((x) => { return (x | 0) === x; })(parseFloat(possibleInteger));
};

const isIntegerWithinBounds = (field, fieldName, lowerBound, upperBound) => {
  let errorMessage = '';
...
const errorOrNext = (errorMessage, res, next) => {
  if (errorMessage) {
    debug(errorMessage);
